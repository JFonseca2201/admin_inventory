...
<template>
    <div>
                ...
                <VRow class="">
                    <VCol cols="10">
                        <VRow>
                            ...
                            <VCol cols="3">
                                <VSelect placeholder="Select" label="Categoría" :items="categories" item-title="name"
                                    item-value="id" v-model="product_categorie_id">
                                </VSelect>
                            </VCol>
                            <VCol cols="3" md="3">
                            ...
                            <VCol cols="4" md="3">
                                ...
                                ]" item-title="title" item-value="id" v-model="disponibilidad">
                                </VSelect>
                            </VCol>
                            <VCol cols="3" md="2">
                                <VSelect placeholder="Select" label="¿Regalo?" :items="[
                                    {
                                        id: 1,
                                        title: 'NO'
                                    },
                                ...
                                ]" item-title="title" item-value="id" v-model="is_gift">
                                </VSelect>
                            </VCol>
                            <VCol cols="4" md="4" >
                                ...
                                <VBtn color="success" class="mx-1" prepend-icon="ri-file-excel-2-line"
                                    @click="downloadExcel()">
                                    <VTooltip activator="parent" location="top">Exportar productos</VTooltip>
                                </VBtn>
                                <VBtn color="primary" class="mx-1" prepend-icon="ri-file-excel-line"
                                    @click="isImportExcelProductDialogVisible = !isImportExcelProductDialogVisible">
                                    <VTooltip activator="parent" location="top">Importar productos</VTooltip>
                                </VBtn>
                ...
                <tbody class="text-uppercase">
                    <tr v-for="item in list_products" :key="item.id">
                        <td>
                            ...
                            <div>
                                <VChip color="primary" v-if="item.state_stock == 1"> Disponible </VChip>
                                <VChip color="warning" v-if="item.state_stock == 2"> Por agotar </VChip>
                                <VChip color="error" v-if="item.state_stock == 3"> Agotado </VChip>
                            </div>
                        </td>
                        <td>
                            {{ item.sku }}
                        ...
                        <td>
                            ...
                            <span v-if="item.is_discount == 2">( {{ item.max_discount }} %)</span>
                        </td>
                        <td>
                            {{ item.importe_iva }} %
                        </td>
                        <td>
                            {{ item.warranty_day }} días.
                        </td>
                        <td>
        ...
        <ProductDeleteDialog v-if="product_selected_delete && isProductDeleteDialogVisible"
            ...
            @deleteUser="addDeleteProduct"></ProductDeleteDialog>
    </div>
</template>