...
<template>
    <div>
            ...
            <VCol md="8">
                                                ...
                                                <VCol cols="12" sm="4">
                                                    <VCard :ripple="false">
                                                        <VCardText class="d-flex flex-column" @click.stop>
                                                            <VImg :src="item.url" width="200px" height="150px"
                                                                class="w-100 mx-auto" />
                        ...
                        <VRow>
                            ...
                            <VCol cols="3" md="3">
                                <VSelect :items="units" item-title="name" item-value="id" placeholder="Select"
                                    v-model="unit_stock_id" label="Unidades" />
                            </VCol>
                            <VCol cols="3" md="3">
                                ...
                                <tr v-for="(product_warehouse, index) in product_warehouses" :key="index">
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            <IconBtn size="small" @click="editItem(product_warehouse)">
                                                <VIcon icon="ri-pencil-line" />
                                            </IconBtn>
                                            <IconBtn size="small" @click="deleteStock(product_warehouse)">
                ...
                <VCard title="Multiples Precios" class="mb-6">
                    <VCardText>
                        <VRow>
                            <VCol cols="4" md="3">
                                <VSelect :items="sucursales" item-title="name" item-value="id" placeholder="Select"
                                    label="Sucursales" v-model="sucursale_price_id" />
                            </VCol>
                            <VCol cols="4" md="3">
                                <VSelect :items="units" item-title="name" item-value="id" placeholder="Select"
                                    label="Unidades" v-model="unit_price_id" />
                            </VCol>
                            <VCol cols="4" md="3">
                                <VSelect :items="[
                                    {
                            ...
                            <VCol cols="3" md="2">
                                <VTextField label="Price" type="number" prefix="$" v-model="price" placeholder="" />
                            </VCol>
                            <VCol cols="1" md="1">
                        ...
                        <VTable>
                            ...
                            <tbody>
                                <tr v-for="(product_wallet, index) in product_wallets" :key="index">
                                    ...
                                    <td>
                                        ${{ parseFloat(product_wallet.price).toFixed(2) }}
                                    </td>
                                    <td>
                                        <div class="flex">
                                            <IconBtn size="small" @click="editProductWalllet(product_wallet)">
                                                <VIcon icon="ri-pencil-line" />
                                            </IconBtn>

                                            <IconBtn size="small" @click="deletePrice(product_wallet)">
                                                <VIcon icon="ri-delete-bin-line" />
                                            </IconBtn>
                                        </div>
                                    </td>
                ...
                <VCard title="Adicionales">
                    <VCardText>
                        <div class="d-flex flex-column gap-y-5">
                            <VSelect placeholder="Select" label="Categoria" :items="categories" item-title="name"
                                item-value="id" v-model="product.product_categorie_id">
                            </VSelect>
                            ...
                            <div class="d-flex">
                                ...
                                <div class="ml-5" v-if="product.is_discount == 2">
                                    <VTextField label="% de descuento maximo:" type="number"
                                        v-model="product.max_discount" placeholder="15%" />
                                </div>
                            </div>

                            <VTextField label="Dias de Garantia:" type="number" v-model="product.warranty_day"
                                placeholder="30" />
                ...
                <VCard title="Especificaciones" class="my-5">
                    <VCardText>
                        <div class="d-flex flex-column gap-y-5">
                            ...
                            <VTextField label="Importe IVA:" type="number" v-model="product.importe_iva"
                                placeholder="15%" />
        ...
        <WarehouseProductEditDialog v-if="warehouse_selected && isEditWarehouseDialog"
            v-model:isDialogVisible="isEditWarehouseDialog" :warehouseSelected="warehouse_selected"
            :warehouses="warehouses" :units="units" :product_id="route.params.id"
            @editWarehouseProduct="warehouseProductEdit">
        </WarehouseProductEditDialog>
        <WarehouseProductDeleteDialog v-if="warehouse_delete_selected && isDeleteWarehouseDialog"
            v-model:isDialogVisible="isDeleteWarehouseDialog" :warehouseSelected="warehouse_delete_selected"
            @deleteWarehouse="warehouseProductDelete"></WarehouseProductDeleteDialog>

        <WalletProductEditDialog v-if="product_wallet_edit_selected && isEditProductWalletDialog"
            v-model:isDialogVisible="isEditProductWalletDialog" :walletSelected="product_wallet_edit_selected"
            :sucursales="sucursales" :units="units" :product_id="route.params.id"
            @editWalletProduct="walletProductEdit">
        </WalletProductEditDialog>
        <WalletProductDeleteDialog v-if="product_wallet_delete_selected && isDeleteProductWalletDialog"
            v-model:isDialogVisible="isDeleteProductWalletDialog" :walletSelected="product_wallet_delete_selected"
            @deleteWallet="walletProductDelete"></WalletProductDeleteDialog>
