...
watch(search_product, query => {
    ...
    if (!warehouse_end_id.value) {
        setTimeout(() => {
            warning_warehouse.value = "Debe seleccionar un almacen de entrega.";
        }, 50);
        return;
    }
...
const store = async () => {
    try {
        ...
        if (!warehouse_start_id.value) {
            setTimeout(() => {
                warning_transport.value = "Se debe elegir un almacen de atención.";
            }, 25);
            return;
        }
        ...
        if (!warehouse_end_id.value) {
            setTimeout(() => {
                warning_transport.value = "Se debe elegir un almacen de entrega.";
            }, 25);
            return;
        }
        if (transport_details.value == 0) {
            setTimeout(() => {
                warning_transport.value = "Se debe ingresar al menos un producto al detalle de entrega.";
            }, 25);
            return;
        }
        ...
        console.log(resp);
        success_transport.value = resp.message_text;
        setTimeout(() => {
            warehouse_start_id.value = null;
            warehouse_end_id.value = null;
            transport_details.value = null;
            description.value = '';
        ...
        <div class="d-flex flex-wrap justify-space-between gap-4 mb-6">
            <div class="d-flex flex-column justify-center">
                ...
                <p class="text-body-1 mb-0">
                    Formulario para registrar una nueva orden de transporte.
                </p>
            </div>
        </div>
        <VRow>
                        ...
                        <VRow>
                            <VCol cols="4">
                                <VSelect v-model="warehouse_start_id" :items="warehouses" label="Almacen de atención"
                                    item-title="name" item-value="id" placeholder="Select Almacen" eager />
                            </VCol>
                            <VCol cols="4">
            ...
            <!-- DETALLE DE LA SOLICITU DE TRANSPORTE -->
            <VCol cols="12">
