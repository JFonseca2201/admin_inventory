...
const addDeleteTransport = (TransportDelete) => {
  let INDEX = list_transports.value.findIndex(
    (transport) => transport.id == TransportDelete.id,
  );
  if (INDEX != -1) {
...
const showPdf = (transport) => {
  window.open(
    import.meta.env.VITE_API_BASE_URL + "transport-pdf/" + transport.id,
    "_blank",
  );
};
  ...
  <div>
    <VCard title="🚚 Transports">
      ...
      <VTable>
        <thead>
          <tr>
            <th class="text-uppercase">N° Transport</th>
            <th class="text-uppercase">Solicitante</th>
            <th class="text-uppercase">Almacen de atención</th>
            <th class="text-uppercase">Almacen de entrega</th>
            <th class="text-uppercase">Fecha de emisión</th>
            <th class="text-uppercase">Estado de E.</th>
