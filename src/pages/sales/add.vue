...
watch(unit_id, (value) => {
  ...
  let PRICE_SELECTED = select_product.value.wallets.find(
    (wallet) =>
...
const config = async () => {
  try {
    ...
    warehouses.value = resp.warehouses.filter(
      (warehouse) => warehouse.sucursale_id == USER_AUTENTICATE.sucursale_id,
    );
    console.log(resp);
  } catch (error) {
    console.log(error);
...
const searchCar = async () => {
  ...
  try {
    const resp = await $api(
      "sales/search_car?search=" + (search_car.value ? search_car.value : ""),
      {
        method: "GET",
        onResponseError({ response }) {
          console.log(response._data.error);
...
const addProduct = () => {
  ...
  if (select_product.value.tax_selected == 2) {
    ...
    IGV =
      conv(price_unit.value) * conv(select_product.value.importe_iva * 0.01);
  }
  let SUBTOTAL = conv(price_unit.value) - conv(discount.value) + IGV;

  let EXIST_PRODUCT_UNIT = sale_details.value.find(
    (sale_detail) =>
      sale_detail.product.id == select_product.value.id &&
      sale_detail.unit_id == unit_id.value,
  );
  if (EXIST_PRODUCT_UNIT) {
    setTimeout(() => {
      warning_warehouse.value =
        "El producto y la unidad ya existe en el detalle.";
    }, 25);
    return;
  }
  if (select_product.value.is_discount == 2) {
    let MAX_DISCONUT =
      select_product.value.max_discount * 0.01 * price_unit.value;
    if (MAX_DISCONUT < discount.value) {
      warning_warehouse.value =
        "El descuento máximo es de $" + conv(MAX_DISCONUT);
      return;
    }
  }
  if (selectedRadio.value == 1 && select_product.value.disponibilidad == 2) {
    let WAREHOSE_PRODUCT = select_product.value.warehouses;
    let WAREHOUSE_SELECTED = WAREHOSE_PRODUCT.find(
      (warehouse) =>
    ...
    if (WAREHOUSE_SELECTED) {
      if (WAREHOUSE_SELECTED.stock < quantity.value) {
        setTimeout(() => {
          warning_warehouse.value =
            "El stock disponible de este producto es de " +
...
const reportDetail = () => {
  ...
  ivg_total.value = sale_details.value.reduce(
    (sum, sale_detail) =>
      sum + Number(sale_detail.igv) * Number(sale_detail.quantity),
    0,
  );
  ...
  discount_total.value = sale_details.value.reduce(
    (sum, sale_detail) =>
      sum + Number(sale_detail.discount) * Number(sale_detail.quantity),
    0,
  );

  //3. SUMA TOTAL DE ORIGINAL
  subtotal_original.value = sale_details.value.reduce(
    (sum, sale_detail) =>
      sum +
      Number(sale_detail.price) /* + Number(sale_detail.igv)*/ *
        Number(sale_detail.quantity),
    0,
  );

  //4. SUMA TOTAL
  total_all.value = sale_details.value.reduce(
    (sum, sale_detail) => sum + Number(sale_detail.total),
    0,
  );
};
const deleteDeatil = (index) => {
  ...
  if (total_all.value - TOTAL_DELETE < payment_total.value) {
    setTimeout(() => {
      warning_warehouse.value =
        "No se puede elminar este producto porque el pago cancelado es mayor. ";
    }, 25);
    return;
  }
  ...
  if (total_all.value <= 0) {
    setTimeout(() => {
      warning_payment.value =
        "El total de la venta debe ser mayor a cero para añadir un pago";
    }, 25);
    return;
  }
  if (
    Number(payment_total.value) + Number(amount.value) >
    Number(total_all.value)
  ) {
    setTimeout(() => {
      warning_payment.value =
        "El total pagado no puede ser mayor al total de la venta";
    }, 25);
    return;
  }
...
const store = async () => {
  ...
  try {
    ...
    if (sale_details.value.length == 0) {
      setTimeout(() => {
        warning_sale.value =
          "Es necesario agrgrar uno o más productos al detallado.";
      }, 25);
      return;
    }
    if (!warehouse_id.value) {
    ...
    if (selectedRadio.value == 1) {
      if (payments.value.length == 0) {
        setTimeout(() => {
          warning_sale.value =
            "Es necesario dar un adelanto para generar la venta.";
        }, 25);
        return;
      }
    }
    ...
    success_sale.value =
      "La " +
      (selectedRadio.value == 1 ? "venta" : "cotización") +
  ...
  <div>
    ...
    <div class="d-flex flex-wrap justify-space-between gap-4 mb-6">
      <div class="d-flex flex-column justify-center">
        <h4 class="text-h4 mb-1">🛍️ Add New Sale / Cotización</h4>
        ...
        <VRow>
            ...
            <VRow>
              <VCol cols="12">
                <VTextField
                  ...
                  @click:append-inner="searchClient"
                  clearable
                />
              </VCol>
              <VCol cols="12 py-0" v-if="client_selected">
                <span
                  ><b>Cliente: &nbsp;</b>{{ client_selected.full_name }}</span
                >

                <br />
                <span
                  ><b>CI/RUC: &nbsp;</b>{{ client_selected.n_document }}</span
                >
                <br />
          ...
          <VCol cols="2" class="d-flex flex-wrap justify-center gap-4">
            <VBtn
              ...
              class="mx-2"
              @click="
                isClientFinalAddDialogVisible = !isClientFinalAddDialogVisible
              "
            >
          ...
          <VCol cols="5">
            <VRow>
              ...
              <VCol cols="12 py-0" v-if="car_selected">
                <VTextField
                  v-if="car_selected && selectedRadio == 1"
                  label="Kilometraje"
            ...
            <VTable class="text-uppercase">
              <thead>
                <tr>
                  ...
                  <th class="text-uppercase">Dcto.</th>
        ...
        <VRow>
          <VCol cols="8" v-if="selectedRadio == 1">
            <VRow>
              <VCol cols="4">
                <VSelect
                  :items="['EFECTIVO', 'DEPOSITO', 'TRANSFERENCIA', 'DE UNA']"
                  item-title="name"
                  item-value="id"
                  placeholder="Select"
    ...
    <ClientSearchDialog
      v-if="list_clients.length > 0 && isClientSearchDialogVisible"
