            ...
            <VCardText>
                <VRow class="">
                    <VCol cols="11">
                        <VRow>
                            <VCol cols="2">
                                <VTextField label="N° venta/cotización" placeholder="N° venta/cotización"
                                    density="compact" class="me-3" v-model="searchQuery" @keyup.enter="list" />
                            </VCol>
                            <VCol cols="2">
                                <VSelect placeholder="Select" label="Tipo de Cliente" density="compact"
                                    :items="[{ id: 1, title: 'Cliente Final' }, { id: 2, title: 'Cliente Empresa' }]"
                                    item-title="title" item-value="id" v-model="type_client">

                                </VSelect>
                            </VCol>
                            <VCol cols="3">
                                <VTextField placeholder="Search Client" density="compact" class="me-3" label="Cliente"
                                    v-model="search_client" @keyup.enter="list" />
                            </VCol>
                            <VCol cols="3">
                                <VTextField placeholder="Search Car" density="compact" class="me-3" label="Vehículo"
                                    v-model="search_car" @keyup.enter="list" />
                            </VCol>
                            <VCol cols="2">
                                <AppDateTimePicker v-model="range_date" label="Rango de fecha" placeholder="Select date"
                                    density="compact" :config="{ mode: 'range' }" />
                        ...
                        <VRow>
                            <VCol cols="2">
                                <VSelect placeholder="Select" label="Tipo" density="compact"
                                    :items="[{ id: 1, title: 'Venta' }, { id: 2, title: 'Cotización' }]"
                                    item-title="title" item-value="id" v-model="type">
                                </VSelect>
                            </VCol>
                            <VCol cols="2">
                                <VSelect placeholder="Select" label="Estado de entrega" density="compact"
                                    :items="[{ id: 1, title: 'Pendiente' }, { id: 2, title: 'Parcial' }, { id: 3, title: 'Completo' }]"
                                    item-title="title" item-value="id" v-model="state_entrega">
                                </VSelect>
                            </VCol>
                            <VCol cols="3">
                                <VSelect placeholder="Select" label="Estado de pago" density="compact"
                                    :items="[{ id: 1, title: 'Pendiente' }, { id: 2, title: 'Parcial' }, { id: 3, title: 'Completo' }]"
                                    item-title="title" item-value="id" v-model="state_payment">
                                </VSelect>
                            </VCol>
                            <VCol cols="3">
                                <VTextField placeholder="Search Product" density="compact" class="me-3" label="Producto"
                            ...
                            <VCol cols="2">
                                ...
                                <div class="d-flex mt-5">
                                    <VBtn color="info" class="mx-1" prepend-icon="ri-search-2-line" @click="list()">
                                        <VTooltip activator="parent" location="top">Buscar venta/cotización</VTooltip>
                                    </VBtn>

                                    <VBtn color="secondary" class="mx-1" prepend-icon="ri-restart-line" @click="reset()">
                                        <VTooltip activator="parent" location="top">Refrescar</VTooltip>
                                    </VBtn>
                                    <VBtn color="success" class="mx-1" prepend-icon="ri-file-excel-2-line"
                                        @click="downloadExcel()">
                                        <VTooltip activator="parent" location="top">Exportar ventas/cotizaciones</VTooltip>
                                    </VBtn>
                                </div>
                            </VCol>
            ...
            <VTable>
                ...
                <tbody class="text-uppercase">
                    <tr v-for="item in list_sales" :key="item.id" style="font-size: smaller;">
                        ...
                        <td style="text-wrap-mode: nowrap;">
                            ${{ parseFloat(item.debt).toFixed(2) }} / ${{ parseFloat(item.paid_out).toFixed(2) }}
                        </td>
                        <td>
                        ...
                        <td>
                            ...
                            <VChip color="warning" v-if="item.state_payment == 2">PARC.</VChip>
                            <VChip color="primary" v-if="item.state_payment == 3">COMP.</VChip>
                        </td>
                        <td>
                            <VChip v-if="item.state_entrega == 1" color="error" prepend-icon="ri-error-warning-line">
                                PEND.
                            </VChip>
                            <VChip v-if="item.state_entrega == 2" color="secondary"
                                prepend-icon="ri-error-warning-line">
                                PARC.
                            </VChip>
                            <VChip v-if="item.state_entrega == 3" color="info" prepend-icon="ri-star-line">
                                ENTR.
                            </VChip>
                        </td>
                        <!-- <td style="text-wrap-mode: nowrap;">
