...
<template>
    <div>
        <VCard title="👥 Clientes">
            <VCardText>
                <VRow class="justify-space-between">
                    <VCol cols="3">
                        <VTextField placeholder="Search Clients" density="compact" class="me-3" v-model="searchQuery"
                            @keyup.enter="list" maxlength="13" clearable /><!-- :rules="rules" -->
                    </VCol>
                    <VCol cols="3" class="">
                        <VRow>
                            <VCol cols="6" class="text-end">
                                <VBtn @click="isClientFinalAddDialogVisible = !isClientFinalAddDialogVisible">
                                    Add Client
                                    <VIcon end icon="ri-add-line" />
                                </VBtn>
                            </VCol>
                            <VCol cols="6" class="text-end">
                                <VBtn @click="isClientCompanyAddDialogVisible = !isClientCompanyAddDialogVisible">
                                    Add Company
                                    <VIcon end icon="ri-add-line" />
                                </VBtn>
                            </VCol>
            ...
            <VTable>
                ...
                <tbody>
                    <tr v-for="client in list_clients" :key="client.id">
                        <td>
                            <span>{{ client.type_document }}</span><br>
        ...
        <ClientFinalEditDialog v-if="client_final_selected_edit && isClientEditDialogVisible"
            v-model:isDialogVisible="isClientEditDialogVisible" :clientSelected="client_final_selected_edit"
        ...
        <ClientCompanyEditDialog v-if="client_company_selected_edit && isClientEditDialogVisible"
            v-model:isDialogVisible="isClientEditDialogVisible" :clientSelected="client_company_selected_edit"
        ...
        <ClientDeleteDialog v-if="client_selected_delete && isClientDeleteDialogVisible"
            v-model:isDialogVisible="isClientDeleteDialogVisible" :clientSelected="client_selected_delete"
