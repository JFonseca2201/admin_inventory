...
<template>
    <div>
        <VCard title="🏤 Providers">
            <VCardText>
                <VRow class="justify-space-between">
                    <VCol cols="3">
                        <VTextField placeholder="Search Providers" density="compact" class="me-3" v-model="searchQuery"
                            @keyup.enter="list" />
                    </VCol>
            ...
            <VDataTable :headers="headers" :items="list_providers" :items-per-page="15" class="text-no-wrap">
                ...
                <template #item.full_name="{ item }">
                    <div class="d-flex align-center">
                        ...
                        <div class="d-flex flex-column ms-3">
                            <span class="d-block font-weight-medium text-high-emphasis text-truncate">{{ item.full_name
                                }}</span>
        ...
        <ProviderAddDialog v-model:isDialogVisible="isProviderAddDialogVisible" @addProvider="addNewProvider">
        </ProviderAddDialog>
        <ProviderEditDialog v-if="provider_selected_edit && isProviderEditDialogVisible"
            ...
            @editProvider="addEditProvider"></ProviderEditDialog>
        <ProviderDeleteDialog v-if="provider_selected_delete && isProviderDeleteDialogVisible"
            ...
            @deleteProvider="addDeleteProvider"></ProviderDeleteDialog>
    </div>
</template>