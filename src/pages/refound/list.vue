...
const list = async () => {
    try {
        let data = {
            ...
            start_date: range_date.value ? range_date.value.split("to")[0].trim() : null,
            end_date: range_date.value ? range_date.value.split("to")[1].trim() : null,
        }
        console.log(data);
...
<template>
    <div>
            ...
            <VCardText>
                <VRow class="">
                    <VCol cols="10">
                        <VRow>
                            <VCol cols="3">
                                <VTextField placeholder="Search N° Venta" density="compact" class="me-3"
                                    v-model="sale_id" @keyup.enter="list" />
                            </VCol>
                            ...
                            <VCol cols="3">
                                <VTextField placeholder="Search Client" density="compact" class="me-3"
                        ...
                        <VRow>
                            ...
                            <VCol cols="3">
                                <VSelect placeholder="Select" label="Tipo" :items="[
                                    {
                                        id: 1,
                                        title: 'Reparar'
                                    },
                            ...
                            <VCol cols="3" v-if="type == 1">
                                ...
                                ]" item-title="title" item-value="id" v-model="state">
                                </VSelect>
                            </VCol>
                            <VCol cols="3">
                            ...
                            <VCol cols="3">
                                <VBtn color="info" class="mx-1" prepend-icon="ri-search-2-line" @click="list()">
                                    <VTooltip activator="parent" location="top">Buscar productos</VTooltip>
                                </VBtn>
                    ...
                    <tr v-for="item in list_refounds" :key="item.id">
                        ...
                        <td>
                            <div class="d-flex gap-1">
                                ...
                                <IconBtn v-if="item.type == 1" size="small" @click="deleteItem(item)">
        ...
        <RefoundAddDialog v-model:isDialogVisible="isRefoundAddDialogVisible" @refoundAddProduct="refoundProductAdd">
        </RefoundAddDialog>
        <RefoundEditDialog v-if="selected_refound_product_edit && isRefoundEditDialogVisible"
            v-model:isDialogVisible="isRefoundEditDialogVisible" :selectedRefoundProduct="selected_refound_product_edit"
            @refoundEditProduct="refoundProductEdit">
        </RefoundEditDialog>
        <RefoundDeleteDialog v-if="selected_refound_product_delete && isRefoundDeleteDialogVisible"
            v-model:isDialogVisible="isRefoundDeleteDialogVisible"
            :selectedRefoundProduct="selected_refound_product_delete" @deleteRefoundProduct="refoundProductDelete">
        </RefoundDeleteDialog>
    </div>
</template>