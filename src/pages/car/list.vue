...
const addEditCar = (editCar) => {
    ...
    let backup = list_cars.value;
    list_cars.value = [];
    let INDEX = backup.findIndex((rol) => rol.id == editCar.id);
    if (INDEX != -1) {
        backup[INDEX] = editCar;
    }
    setTimeout(() => {
        list_cars.value = backup;
    }, 50);
...
const addDeleteCar = (Car) => {
    ...
    let backup = list_cars.value;
    list_cars.value = [];
    let INDEX = backup.findIndex((rol) => rol.id == Car.id);
    if (INDEX != -1) {
...
<template>
    <div>
        <VCard title="🚗 Cars">
            <VCardText>
                <VRow class="justify-space-between">
                    <VCol cols="3">
                        <VTextField placeholder="Search Car" density="compact" class="me-3" v-model="searchQuery"
                            @keyup.enter="list" />
                    </VCol>
            ...
            <VDataTable :headers="headers" :items="list_cars" :items-per-page="10" class="text-no-wrap">
                <template #item.id="{ item }">
                    <span class="text-h6">{{ item.id }}</span>
                </template>
                <template #item.brand="{ item }">
                    <span class="text-h6">{{ item.brand }}</span>
                </template>
                <template #item.model="{ item }">
                    <span class="text-h6">{{ item.model }}</span>
                </template>
                <template #item.type_car="{ item }">
                    <span class="text-h6">{{ item.type_car }}</span>
                </template>
                <template #item.placa="{ item }">
                    <span class="text-h6">{{ item.placa }}</span>
                </template>
                ...
                <template #item.observation="{ item }">
                    <span class="text-h6; text-no-wrap">{{ item.observation }}</span>
                </template>
        ...
        <CarsEditDialog v-if="car_selected_edit && isCarsEditDialogVisible"
            v-model:isDialogVisible="isCarsEditDialogVisible" :carSelected="car_selected_edit" @editCar="addEditCar">
        </CarsEditDialog>
        <CarDeleteDialog v-if="car_selected_delete && isCarDeleteDialogVisible"
            v-model:isDialogVisible="isCarDeleteDialogVisible" :carSelected="car_selected_delete"
