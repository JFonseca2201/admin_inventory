...
const store = async () => {
    try {
        ...
        if (!quantity.value || quantity.value <= 0) {
            setTimeout(() => {
                warning.value = "Es requerido que la cantidad sea mayor a cero.";
            }, 25);
            return;
        }
        if (!description.value) {
            setTimeout(() => {
                warning.value = "Es requerido digitar una razón de la devolución.";
            }, 25);
            return;
        }
        let data = {
        ...
        console.log(resp);
        success.value = "La devolución se editó exitosamente!";
...
onMounted(() => {
    sale_id.value = props.selectedRefoundProduct.sale_id;
            ...
            <VCardText class="pt-5">
                <div class="text-center pb-6">
                    <h4 class="text-h4 mb-2">
                        Edición de Devolución: {{ props.selectedRefoundProduct.id }}
                    </h4>
                </div>
                ...
                <VForm class="mt-4" @submit.prevent="store">
                    <VRow>
                        <VCol cols="4">
                            ...
                            ]" item-title="title" item-value="id" v-model="type" disabled>
                            </VSelect>
                        </VCol>
                        <VCol cols="4" v-if="props.selectedRefoundProduct.type == 1">
                        ...
                        <VCol cols="4" v-if="props.selectedRefoundProduct.type == 1">
                            <VTextField label="Cantidad" placeholder="" type="number" v-model="quantity" />
                        </VCol>
