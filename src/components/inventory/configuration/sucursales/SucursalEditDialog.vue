...
const store = async () => {
  ...
  if (!ruc.value) {
    setTimeout(() => {
      warning.value = "Se debe llenar el RUC de la sucursal";
    }, 50);
    return;
  }
  if (!email.value) {
    setTimeout(() => {
      warning.value = "Se debe llenar el correo electrónico de la sucursal";
    }, 50);
    return;
  }
...
onMounted(() => {
  ...
  ruc.value=props.sucursalSelected.ruc;
  telefono.value=props.sucursalSelected.telefono;
  email.value=props.sucursalSelected.email;
