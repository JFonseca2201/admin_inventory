...
const update = async () => {
    ...
    if (!surname.value) {
        setTimeout(() => {
            warning.value = "Se debe llenar la apellido del cliente";
        }, 50);
        return;
    }
    if (!n_document.value) {
        setTimeout(() => {
            warning.value = "Se debe llenar el No de documento del cliente";
        }, 50);
    ...
    if (!type_document.value) {
        setTimeout(() => {
            warning.value = "Se debe seleccionar un tipo de documentoel cliente";
        }, 50);
        return;
    }
    if (!phone.value) {
        setTimeout(() => {
            warning.value = "Se debe llenar la teléfono del cliente";
        }, 50);
        return;
    }
    ...
    if (!ubigeo_region.value) {
        setTimeout(() => {
            warning.value = "Se debe seleccionar una región para el cliente";
        }, 50);
        return;
        ...
        setTimeout(() => {
            warning.value = "Se debe seleccionar una ciudad para el cliente";
        }, 50);
        return;
    }
    ...
    if (!address.value) {
        setTimeout(() => {
            warning.value = "Se debe llenar la dirección del cliente";
        }, 50);
        return;
    }
    ...
    try {
        ...
        if (resp.message == 403) {
            ...
            n_document.value = '';
            address.value = '';
            type_document.value = "CI"
            gender.value = "M";
            birth_date.value = null;
            ubigeo_region.value = null;
            ubigeo_provincia.value = null;
            ubigeo_distrito.value = null; */
            /* warning.value = null;
            error_exits.value = null;
            success.value = null; */
            //onFormReset();
            ...
            <VCardText class="pt-5">
                <div class="text-center pb-6">
                    <h4 class="text-h4 mb-2">
                        Edit Client Final: {{ props.clientSelected.full_name }}
                    </h4>
                </div>
                ...
                <VForm class="mt-4" @submit.prevent="update">
                    <VRow>
                        <!-- 👉 First Name -->
                        <VCol cols="6">
                            <VSelect :items="['CI', 'RUC', 'PASAPORTE']" v-model="type_document"
                                label="Tipo de documento" placeholder="Select Item" eager />
                        </VCol>
                        <VCol cols="6">
                            <VTextField v-model="n_document" type="number" label="Cédula"
                                placeholder="Example: 0999999999" />
                        </VCol>

                        <VCol cols="6">
                            <VTextField v-model="name" label="Nombre del Cliente" placeholder="Example: Jose" />
                        </VCol>
                        <VCol cols="6">
                            <VTextField v-model="surname" label="Apellido del Cliente" placeholder="Example: Perez" />
                        </VCol>
                        <VCol cols="6">
                            <VSelect :items="[
                                {
                        ...
                        <VCol cols="6">
                            ...
                            ]" item-title="name" item-value="id" v-model="state" label="Estado" placeholder="Select Item" eager />
                        </VCol>
                        <VCol cols="6">
