...
const emit = defineEmits(["update:isDialogVisible", "importExcelProduct"]);
...
const loadFile = ($event) => {
  if (
    $event.target.files[0].type.indexOf(
      "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    ) < 0
  ) {
    error_exits.value = "SOLAMENTE PUEDEN SER ARCHIVOS DE TIPO EXCEL";
    FILE_EXCEL.value = null;
    return;
  }
  error_exits.value = "";
    ...
    <VCard class="pa-sm-11 pa-3">
      ...
      <VCardText class="pt-5">
        <div class="text-center pb-6">
          <h4 class="text-h4 mb-2">Import Excel Product</h4>
        </div>
        ...
        <VForm class="mt-4" @submit.prevent="upload">
          <VRow>
            ...
            <VCol cols="12" class="d-flex flex-wrap justify-center gap-4">
              <VBtn type="submit"> Importar </VBtn>
